...
import { addTask, clearFocus, deleteTask, editTask, findTask, focusTask, focusedBrief, initProject, listTasks, MAX_CONTEXT_BYTES, validateTitle } from "./store.ts";
const HELP = [
  "/focus init — set up private task storage; migrate an old brief if present",
  ...
  "/focus clear — clear the focus without deleting the saved task",
...
export default function focusTaskExtension(pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", (event, ctx) => {
    try {
      const brief = focusedBrief(ctx.cwd);
      if (brief) event.systemPromptOptions.contextFiles.push(brief);
    } catch (error) {
      ...
      try {
        ...
        switch (action) {
          case "init": {
            ...
            report(ctx, result.migrated ? "Migrated old brief into private task storage; remove the old tracked file and its AGENTS.md read instruction yourself." :
              result.legacy ? "Existing legacy brief left untouched; resolve it before creating a new focus." : "Private task storage ready; AGENTS.md unchanged.");
            break;
          }
          case "add": {
          ...
          case "switch": {
            ...
            requireIdle(ctx);
            const { task } = focusTask(ctx.cwd, findTask(tasks, query).id);
            report(ctx, `Focused ${task.id.slice(0, 8)} — ${task.title}\nFor a clean conversation, use /new; the focus persists.`);
            break;
          }
          case "list": {
            const { tasks, active } = listTasks(ctx.cwd);
          ...
          case "edit": {
            const { mode, text } = draftOptions(argument);
            if (!ctx.hasUI && !text && mode !== "polish") throw new Error("Edit the active .pi-focus-task/<id>.md or use /focus edit --raw <brief> in non-interactive mode.");
            ...
            report(ctx, "Updated focused task.");
            break;
          }
          case "clear": {
            clearFocus(ctx.cwd);
            report(ctx, "Focus cleared; saved task retained.");
            break;
          }
          case "delete": {
